...
from source.apps.tmdb.utils import Show, fetch, format_episode_label, get_air_date


class TestShow:
    @pytest.fixture
    def mock_set_progress_related(self, mocker):
        return mocker.patch("source.apps.tmdb.utils.Show._set_progress_related")
    ...
    def test_aired_episodes(self, mocker, mock_set_progress_related):
        mocker.patch(
            "source.apps.tmdb.utils.Show._seasons",
            new=mocker.PropertyMock(return_value=[{"episode_count": 3}, {"episode_count": 3}]),
        )
        mocker.patch(
            "source.apps.tmdb.utils.Show.last_aired_episode",
...
class TestFetch:
    ...
    def httpx(self, mocker, response):
        httpx = mocker.patch("source.apps.tmdb.utils.httpx")
...
class TestGetAirDate:
    def test_with_air_date(self, mocker):
        fetch_mock = mocker.patch(
            "source.apps.tmdb.utils.fetch", return_value={"air_date": "2019-6-30"}
        )
    ...
    def test_without_air_date(self, mocker, fetched_data):
        fetch_mock = mocker.patch("source.apps.tmdb.utils.fetch", return_value=fetched_data)
